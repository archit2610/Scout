...
import { retrieveRelevantChunks, buildContextBlock, storeReportMemory } from "./embedding.service.js";
export const runScout = async ({ question, conversationId, reportId, emit, }) => {
    let retrievedChunks = [];
    if (conversationId) {
    ...
    if (plan.needsWebSearch) {
        ...
        searchContext = await withTimeout(fetchAndExtract(uniqueResults, question), 20000, "");
