...
export const runResearch = async (reportId, question, emit) => {
    try {
        ...
        const contexts = await Promise.all(subQuestions.map(async (subQ) => {
            ...
            const extracted = await Promise.all(results
                .slice(0, 3)
                .map(result => fetchAndExtract(result.url, subQ, reportId)));
            return extracted.filter(Boolean).map(r => `Source: ${r.url}\n${r.facts}`).join("\n\n");
        }));
